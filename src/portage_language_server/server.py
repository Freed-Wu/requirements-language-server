...
def check_extension(
    uri: str,
) -> Literal["ebuild", "make.conf", "color.map", ""]:
    r"""Check extension.

    :param uri:
    :type uri: str
    :rtype: Literal["ebuild", "make.conf", "color.map", ""]
    """
    if uri.split(os.path.extsep)[-1] in ["ebuild", "eclass"]:
        return "ebuild"
    if os.path.basename(uri) == "make.conf":
        return "make.conf"
    if os.path.basename(uri) == "color.map":
        return "color.map"
    return ""
...
def get_document(
    ...
    r"""Get document. ``builtin`` will use builtin portage.json. ``cache``
    will generate a cache from ``${XDG_CACHE_DIRS:-/usr/share}
    /man/``. ``system`` is same as ``cache`` except it doesn't
    generate cache. Some distribution's portage doesn't contain textinfo. So
    ...
    if method == "builtin":
        ...
        if not os.path.exists(user_cache_dir("portage.json")):
            document = init_document()
            with open(user_cache_dir("portage.json"), "w") as f:
                json.dump(document, f)
        else:
            with open(user_cache_dir("portage.json"), "r") as f:
                document = json.load(f)
    else:
        from .api import init_document
...
class PortageLanguageServer(LanguageServer):
    r"""Portage language server."""
        ...
        def completions(params: CompletionParams) -> CompletionList:
            ...
            items = [
                CompletionItem(
                    ...
                    if x.isupper() or self.document[x][1] != "ebuild"
